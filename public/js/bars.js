    ...
    $(this).attr('show_values') == 'true' &&
      ...
      void 0 !== $(this).attr('unit')
        ? ($(this).append(
          ...
          $(this).append(
            `<p class="bar_label_max">${$(this).parent().data('bg_max')} ${$(
              this
            ).attr('unit')}</p>`
          ))
        : ($(this).append('<p class="bar_label_min">0</p>'),
          $(this).append(
            `<p class="bar_label_max">${$(this).parent().data('bg_max')}</p>`
          )));
  });
}
function show_tooltips() {
  $('.bar_group__bar').each(function () {
    $(this).attr('tooltip') == 'true' &&
      ($(this).css('margin-bottom', '40px'),
      $(this).append(
        `<div class="b_tooltip"><span>${$(this).attr(
          'value'
